...
KOS_neighbor_counties <- c("Fulton", "Marshall", "Saint Joseph", "Elkhart", "LaGrange", "Noble", "Whitley", "Allen", "Huntington", "Wabash", "Miami")
...
ggplot(IN_Hosp, aes(x = date, y = difhosp_per)) + 
  ...
  xlab("Date") +
  stat_smooth(data = subset(IN_Hosp, date < yest), size = 1, se = FALSE, linetype = "dashed")
```
...
#  scale_y_log10(breaks = breaks, minor_breaks = minor_breaks) + 
  scale_y_continuous(limit=c(0,220), oob = squish) +
...
ggplot(IN_outbreak, aes(x = date, y = difdeaths_per, color = county)) + 
  ...
  xlab("Date") +
  stat_smooth(size = 1, se = FALSE, linetype = "dashed") +
  facet_wrap(vars(county), nrow = 2, ncol = 2)
```
...
```{r, echo=FALSE, fig.width=12, fig.height=12, warning=FALSE, message=FALSE}
...
### Cases For Each Neighboring State
...
### Comparison of Smoothed Weighted Average of States
...
### Confirmed Deaths for Each State
