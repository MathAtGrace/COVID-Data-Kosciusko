...
Notes and Irregularities:
  ...
  - (5/14) Vaccine percentages are now based on population 12 and over.  Previously, they were based on population 16 and over.
  - (4/19) Added two pages at the end with vaccine information for the state and for local zip codes.
...
vacs <- read.csv(url)%>%
  mutate(zip_cd = as.character(zip_cd),
         ...
         partial = case_when(
           (first + single) > 0 ~ (first + single)/population_12_and_over,
           TRUE ~ as.numeric(NA)
         )
  )
...
The following map shows percentages of adults who have had at least one vaccine shot.  Percentages are based on population census data from 10-11 years ago.
