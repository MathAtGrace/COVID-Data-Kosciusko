...
SS <- S_spring %>%
  ...
  select(id:first_name, quarantine_isolation_location,
         travel_quarantine_start_date:test_result,
         actual_release_from_quarantine_isolation:actual_release_from_quarantine_isolation,
         complete) %>%
  ...
  mutate(test_result = factor(case_when(
           !is.na(asymptomatic) ~ "p",
           TRUE ~ tolower(substring(test_result, 1, 1))))) %>%
  mutate(type = factor(case_when(
           test_result == "p" ~ "Isolation",
           !is.na(symptoms) ~ "Isolation", #Don't need? !is.na(asymptomatic) ~ "Isolation",
           TRUE ~ "Quarantine"))) %>%
  mutate(started = case_when(
    is.na(started) & test_result == "p" & !is.na(exposure) ~ exposure,
  ...
  mutate(full = factor(str_glue("{first_name} {last_name}", na_action="ignore")),
    id = case_when(
    is.na(id) ~ as.numeric(full),
    TRUE ~ id)) %>%
...
#All Reported
TA <- T2 %>%
  group_by(test_result, .drop = FALSE) %>%
  count()%>%
  pivot_wider(names_from = test_result, values_from = n) %>%
  mutate(Type = "Spring: All Reported",
         Total = n + p,
...
#Weekly  
TW <- T2 %>%
  filter(!at_health_center) %>%
  group_by(test_result, .drop = FALSE) %>%
  count() %>%
  pivot_wider(names_from = test_result, values_from = n) %>%
  mutate(Type = "Spring: All Weekly",
         Total = n + p,
         Percent = percent(case_when(
           Total == 0 ~ 0,
           TRUE ~ p/Total
...
#Past Week ALl Reported
PTA <- T2 %>%
  filter(test_date > yest - 7) %>%
  group_by(test_result, .drop = FALSE) %>%
  count() %>%
  pivot_wider(names_from = test_result, values_from = n) %>%
  mutate(Type = "Past Week: All Reported",
         Total = n + p,
         Percent = percent(case_when(
           Total == 0 ~ 0,
           TRUE ~ p/Total
...
PTW <- T2 %>%
  filter(!at_health_center & test_date > yest - 7) %>%
  group_by(test_result, .drop = FALSE) %>%
  count() %>%
  pivot_wider(names_from = test_result, values_from = n) %>%
  mutate(Type = "Past Week: All Weekly",
         Total = n + p,
         Percent = percent(case_when(
           Total == 0 ~ 0,
           TRUE ~ p/Total
...
T_Tab <- bind_rows(PTW, PTA, TW, TA)

TbyD <- T2 %>%
  mutate(at_health_center = factor(at_health_center)) %>%
  group_by(test_date, at_health_center, test_result, .drop = FALSE) %>%
  count() %>%
  group_by(test_result, at_health_center) %>%
  complete(test_date = seq.Date(as.Date("2021-01-12"), yest, by = "day"),
           fill = list(n = 0))%>%
  pivot_wider(names_from = test_result, values_from = n) %>%
  mutate(n = roll_sumr(n, 7, fill = NA),
         p = roll_sumr(p, 7, fill = NA)) %>%
  drop_na()
...
TbyDA <- TbyD %>%
  ...
  ungroup() %>%
  mutate(Type = "All Reported")

TbyDW <- TbyD %>%
  filter(at_health_center == "FALSE") %>%
  ungroup() %>%
  mutate(Type = "Weekly") %>%
  select(Type, test_date, n, p)

T_Pos <- bind_rows(TbyDA, TbyDW) %>%
  mutate(Total = n + p,
         Percent = percent(case_when(
           Total == 0 ~ 0,
           TRUE ~ p/Total
...
c_vals2 = gg_color_hue(3)
names(c_vals2) <- c("Other Reported", "Weekly", "All Reported")
```
...
Notes and Irregularities:
  ...
  - (1/25) The positivity rate is coming out really high for all reported tests right now because this report does not have access to all the the negative tests that students have reported before coming to campus.
  - (1/25) The methods of this report had to be redone in order to work with the format of the new contact tracing document.  For some reason, these changes caused several quarantines from the fall to be classified as isolations or not counted.  I will take a closer look at this soon. (Ryan)
...
T_Tab %>%
  kbl(caption = "Student Testing",
      ...
      col.names = c("Type", "Tests", "Percent Postive"),
      align = c('l','r','r')) %>%
  kable_styling(latex_options = "striped")%>%
  kable_styling(latex_options = "hold_position")
```

```{r student_testing_percent, fig.height=5}
T_Pos %>%
  filter(Type == "Weekly" | test_date > as.Date("2021-01-24")) %>%
  mutate(Percent = as.numeric(sub("%","",Percent))/100) %>%
  ggplot(aes(x = test_date, y = Percent, color = Type)) +
  geom_point(size = 3)+
...
```{r student_testing, fig.height=5}
T2 %>%
  mutate(at_health_center = factor(at_health_center)) %>%
  group_by(test_date, at_health_center, .drop = FALSE) %>%
  count() %>%
  pivot_wider(names_from = at_health_center, values_from = n) %>%
  rename(Weekly = "FALSE", 'Other Reported' = "TRUE") %>%
  pivot_longer(cols = Weekly:'Other Reported', names_to = "Type") %>%
  ggplot(aes(x = test_date, y = value, fill = Type)) +
